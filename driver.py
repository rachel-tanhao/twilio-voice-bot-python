...
# def get_memory_context(phone_number):
#     """
#     For testing purposes, return a mock memory context that includes a preferred time 30 seconds from now.
#     """
#     future_time = datetime.now() + timedelta(seconds=30)
#     time_str = future_time.strftime("%I:%M %p")  # Format time as HH:MM AM/PM
#     test_message = {
#         'role': 'user',
#         'content': f"Please call me back at {time_str}"
#     }
#     return [test_message]
...
def get_preferred_call_time(phone_number):
    ...
    """
    memory_context = get_memory_context(phone_number)
...
if __name__ == '__main__':
    ...
    async def main():
        await make_call(TEST)
    asyncio.run(main())
    # scheduler = AsyncIOScheduler()
    
    # # For testing: Schedule a one-time call 10 seconds from now
    # run_time = datetime.now() + timedelta(seconds=10)
    
    # # Only schedule if no recent calls
    # existing_jobs = [
    #     job for job in scheduler.get_jobs() 
    #     if job.args == [TEST] and job.next_run_time > datetime.now()
    # ]
    ...
    #         args=[TEST]
    #     )
    #     print(f"Scheduled test call for {TEST} at {run_time.strftime('%H:%M:%S')}")
    # else:
    #     print("Skipping schedule - call already planned")
